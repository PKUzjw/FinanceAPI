...
! !       Apath,Bpath,Dpath,Hpath,Mpath,Cpath,mapath,mbpath,mdpath,mhpath,mmpath,mcpath, &
! !       napath,nbpath,ndpath,nhpath,nmpath,ncpath)
  subroutine allocation(theta,beta,gamma,ageini,numearner,numchild,income, &
       muH,sigma_occu,sigma_sector,Medexp,muY,&
  ...
  integer, parameter :: singlep = kind(1.0), doublep = kind(1.0d0)
  integer, parameter :: fp = singlep !doublep
  real(fp),intent(in):: theta,beta,gamma,income,muH,sigma_occu,sigma_sector,Medexp,muY
  ...
  real(fp)::Yprofiletot0(70), Yprofile(nT),agein,cbar
  real(fp):: Rhvec(3), Ravec(3), Yvec(3)
  ...
  integer ::t,i,j,model,iy,ira,irh,ttoT,ii,Tspan,tpast
  real(fp),dimension(nT-ageini+21,7)::path6
  ...
  integer,parameter::nY=3,nEps=3,nRh=3,nRa=3
  real(fp)::Ytran(nY,nY),Rhtran(nRh,nRh),pRavec(nRa),Ratran(nRa,nRa)
  real(fp), parameter :: sigEta=0.015000**0.5000, rho=0.95000  !permanent shocks
  real(fp), parameter :: sigEps=0.2000**0.5000                !transitory income shocks
  real(fp), parameter :: sigStk=0.469804427065292000         !stock return shocks
  real(fp), parameter :: sigH=0.1000, rhoH=0.30
  ...
  !print*,'stock=',stock,';  bond=',bond,';   deposit=',deposit
  !print*,'house=',house,';  mortgage=',mortgage,'; cbar=',cbar
  ! gamma=2
   ! theta=0.3
   ! beta=0.96
   ! income=1.0
  ...
  Yprofiletot0= &
   ...
   ! Ravec= (/0.30939,  1.00000,  1.69061/)
    ...
    do t=agein,nT
       ...
       path6(t-agein+1,:)=(/a1,b1,c1,d1,h1,m1,adj/)
       a0 = a1*(muA+Ravec(ira))
       b0 = b1*Rb+d0*Rd
       d0 = d1*Rd
       h0 = h1*(muH+hreturn)
       m0 = m1*rM
    enddo
    apath = path6(:,1)*income
    bpath = path6(:,2)*income
    cpath = path6(:,3)*income
    dpath = path6(:,4)*income
    hpath = path6(:,5)*income
    mpath = path6(:,6)*income
